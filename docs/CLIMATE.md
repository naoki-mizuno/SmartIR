...
| `always_send_command`        | boolean | optional | If `true`, always send the `on`/`off` command, even if SmartIR's cached state already matches the requested state (normally skipped to avoid double-toggling devices that use the same IR code for `on` and `off`). Use this if the cached state can drift from the real device state and commands stop getting sent. Default is `false`.                                                                                                 |
